...
if __name__ == "__main__":
    ...
    for i in range(0, 5):
        ...
        net_idx = split3.index('network')
        lo_ip_prefix = str(ipaddress.ip_network(split3[net_idx+1] + '/' + split3[net_idx+2], strict=False)
                           .prefixlen)
        mgm_ip_prefix = str(ipaddress.ip_network(split3[net_idx+6] + '/' + split3[net_idx+7], strict=False)
                            .prefixlen)
        ospf_area = split3[net_idx+4]
        ospf_networks = split3[net_idx+1] + '/' + lo_ip_prefix, split3[net_idx+6] + '/' + mgm_ip_prefix
